...
bg.scale=2;
 ...
 //creating player
  ...
  woman_running=createSprite(150,270);
   ...
   woman_running.x=150
   woman_running.setCollider("rectangle",0,0,woman_running.width,woman_running.height);
...
function draw(){
 ...
 //moving background
  ...
  woman_running.visible=true;
    ...
    // random objects
   var select_object = Math.round(random(0,4));
...
function spawnMask() {
   ...
   mask.scale = 0.2;
   maskGroup.add(mask);
   mask.setCollider("rectangle",0,0,mask.width,mask.height)
   mask.debug=false;
    }
   ...
   sanitizer.setCollider("rectangle",0,0,sanitizer.width,sanitizer.height)
   sanitizer.debug=false;
  }
  ...
  function spawnHighFive() {
   ...
   highFive.setCollider("rectangle",0,0,highFive.width,highFive.height)
   highFive.debug=false;
  }
  ...
  crowd.setCollider("rectangle",0,0,crowd.width,crowd.height)
  crowd.debug=false;
  }
